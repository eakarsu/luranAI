...
export default function CampaignDetailPage() {
  ...
  if (error || !campaign) {
    return (
      <div className="bg-red-50 border border-red-200 rounded-xl p-6 text-red-700">
        ...
        <button onClick={() => router.push('/campaigns')} className="mt-3 text-sm underline">
          Back to Campaigns
        </button>
      </div>
    )
  ...
  const responseRate = campaign.sentCount > 0 ? ((campaign.responseCount / campaign.sentCount) * 100).toFixed(1) : '0'
    ...
    <div className="max-w-3xl mx-auto">
      ...
      {!editing && (
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 text-center">
            <p className="text-2xl font-bold text-blue-600">{campaign.openCount.toLocaleString()}</p>
            <p className="text-xs text-gray-500 mt-1">Opens ({openRate}%)</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 text-center">
            <p className="text-2xl font-bold text-green-600">{campaign.responseCount.toLocaleString()}</p>
            <p className="text-xs text-gray-500 mt-1">Responses ({responseRate}%)</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4 text-center">
            <p className="text-2xl font-bold text-purple-600">{campaign.conversionCount.toLocaleString()}</p>
            <p className="text-xs text-gray-500 mt-1">Conversions ({conversionRate}%)</p>
          </div>
        </div>
      )}
          ...
          <div className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="text-gray-900 font-medium">{campaign.audience}</p>
              </div>
              <div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1 mt-1">
                  {Array.isArray(campaign.channels) && campaign.channels.map((ch) => (
                    <span key={ch} className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-primary-100 text-primary-800">
                      {ch}
                    </span>
                  ))}
                </div>
