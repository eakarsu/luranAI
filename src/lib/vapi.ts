...
RULE: Once you switch to a persona, stay in that persona until the call ends. Do NOT switch back to the general assistant voice. Do NOT blend personas.`
  ...
  if (data.status === 'ended' && data.endedReason) {
    throw new Error(`Vapi call ended immediately: ${data.endedReason}`)
  }
...
export async function getVapiPhoneNumber(): Promise<any | null> {
  if (!VAPI_API_KEY || !VAPI_PHONE_NUMBER_ID) return null
  try {
    const res = await fetch(`${VAPI_API_URL}/phone-number/${VAPI_PHONE_NUMBER_ID}`, {
      headers: { Authorization: `Bearer ${VAPI_API_KEY}` },
    })
    if (!res.ok) return null
    return await res.json()
